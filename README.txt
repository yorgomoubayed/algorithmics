...
The repository contains two Python scripts and a text file:
	-bwt_script.py.
	-huffman_script.py.
	-main_script.py.
	-main_test_file.txt
...
To run the scripts, the user has two options:
	-Install a Python IDE with a Python 3+ installation to manually input the string.
...
The user is provided with a default string "ATGCTAGTTTACGTCCGGTTCAGC". To change it, he needs to manually input his string in the "input_string" of the if __name__ == "main" block on line 83. When executed, the user gets the following outputs printed in the terminal:
...
The user is provided with a default string "ATGCTAGTTTACGTCCGGTTCAGC". To change it, he needs to manually input his string in the "sequence" of the if __name__ == "main" block on line 184. 
...
The main_sctipt combines both the Huffman coding and the Burrows-Wheeler transform Python implementations (check other scripts for details). 
The main difference is that through the script, the user is able to compress/decompress data that is contained in a file as well as saving the BWT resluts.
